        ...
        float frequency, float ps_duty, float lead_dt, float lag_dt)
{
    ...
    pspwm_up_down_ctr_mode_set_frequency(mcpwm_num, frequency, lead_dt, lag_dt);
 ...
 * float lag_dt: lagging bridge-leg dead-time in sec (0..)
 */
esp_err_t pspwm_up_down_ctr_mode_set_frequency_dt(
        mcpwm_unit_t mcpwm_num,
        float frequency, float lead_dt, float lag_dt)
{
    ...
    DBG("Call pspwm_up_down_ctr_mode_set_frequency, period register: %d", timer_top);
    portENTER_CRITICAL(&mcpwm_spinlock);
    // Register 16.17: PWM_GEN0_TSTMP_A_REG (0x0040) etc.
    // also for GEN1 with different register offset
    MCPWM[mcpwm_num]->channel[MCPWM_TIMER_0].cmpr_value[MCPWM_OPR_A].cmpr_val = cmpr_a_lead;
    MCPWM[mcpwm_num]->channel[MCPWM_TIMER_1].cmpr_value[MCPWM_OPR_A].cmpr_val = cmpr_a_lag;
    // Register 16.18: PWM_GEN0_TSTMP_B_REG (0x0044) etc.
    // also for GEN1 with different register offset
    MCPWM[mcpwm_num]->channel[MCPWM_TIMER_0].cmpr_value[MCPWM_OPR_B].cmpr_val = cmpr_b_lead;
    MCPWM[mcpwm_num]->channel[MCPWM_TIMER_1].cmpr_value[MCPWM_OPR_B].cmpr_val = cmpr_b_lag;
    // Register 16.2: PWM_TIMER0_CFG0_REG (0x0004) etc.
    MCPWM[mcpwm_num]->timer[MCPWM_TIMER_0].period.period = timer_top;
    MCPWM[mcpwm_num]->timer[MCPWM_TIMER_1].period.period = timer_top;
    portEXIT_CRITICAL(&mcpwm_spinlock);
    return ESP_OK;
}
 ...
 */
esp_err_t pspwm_4x_deadtime_init(
 ...
 * float lag_fed: dead time value for falling edge, lagging leg 
 */
esp_err_t pspwm_4x_deadtime_set_frequency(
 ...
 * float ps_duty: Duty cycle of the rectified waveform (0..1)
 */
esp_err_t pspwm_4x_deadtime_set_ps_duty(mcpwm_unit_t mcpwm_num, float ps_duty)
{
    portENTER_CRITICAL(&mcpwm_spinlock);
    // Phase shift value is based on timer 0 period setting but intentionally
    // only set for timer 1. Timer 0 is the reference phase.
 ...
 * float lag_fed: dead time value for falling edge, lagging leg
 */
esp_err_t pspwm_4x_deadtime_set_deadtimes(
